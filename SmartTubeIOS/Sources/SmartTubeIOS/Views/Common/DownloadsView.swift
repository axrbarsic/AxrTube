...
/// The only owner of AxrTube offline media and storage controls.
struct DownloadsView: View {
    ...
    private var collectionHeader: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .firstTextBaseline) {
                SmartTubeMatrixHeader(title: "Downloads")
                ...
                Menu {
                    ...
                    Image(systemName: "ellipsis")
                        ...
                        .background(SmartTubeVisualTokens.panelElevated, in: Circle())
                }
    ...
    private var emptyState: some View {
        ...
        .padding(28)
        .smartTubeCardSurface(cornerRadius: 20, contentPadding: 18)
        .padding(.horizontal, SmartTubeVisualTokens.horizontalPadding)
...
private struct DownloadedNowPlayingCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                ...
                Button(action: onPlayPause) {
                    Image(systemName: isPlaying ? "pause.fill" : "play.fill")
                        ...
                        .frame(width: 44, height: 44)
                        .background(SmartTubeVisualTokens.mint, in: Circle())
                }
                .buttonStyle(.plain)
            ...
            if entry.status == .failed || entry.status == .cancelled {
                Text(failureDescription(entry))
                    .font(.caption)
                    .foregroundStyle(SmartTubeVisualTokens.warning)
                    .lineLimit(2)
        ...
        .smartTubeCardSurface(cornerRadius: 18, contentPadding: 14)
        .overlay(alignment: .topLeading) {
            Text("Now Playing")
                ...
                .padding(.vertical, 4)
                .background(SmartTubeVisualTokens.mint, in: Capsule())
                .offset(x: 12, y: -8)
        }
            ...
            ZStack {
                GeometryReader { proxy in
                    ZStack(alignment: .leading) {
                        Capsule().fill(SmartTubeVisualTokens.stroke)
                        Capsule()
                            .fill(SmartTubeVisualTokens.mintSoft.opacity(0.55))
...
private struct DownloadedMediaRow: View {
    ...
    var body: some View {
        ...
        .smartTubeCardSurface(cornerRadius: 16, contentPadding: 12)
    }
        ...
        case .failed, .cancelled:
            VStack(alignment: .leading, spacing: 6) {
                ...
                Button("Retry", action: onRetry)
                    ...
                    .tint(SmartTubeVisualTokens.mint)
                    .foregroundStyle(.black)
                    .smartTubeEDRPressEffect(enabled: edrEnabled, cornerRadius: 8)
            }
        default:
            HStack(spacing: 8) {
...
private func mediaThumbnail(entry: DownloadedVideo, width: CGFloat, height: CGFloat) -> some View {
    AsyncImage(url: entry.thumbnailURL) { phase in
        if case .success(let image) = phase {
            ...
            Rectangle().foregroundStyle(SmartTubeVisualTokens.panelElevated)
                .overlay(Image(systemName: entry.kind == .audio ? "waveform" : "film").foregroundStyle(SmartTubeVisualTokens.mintSoft))
        }
    }
