...
private struct SmartTubePressPulseEvent: Identifiable {
    let id = UUID()
    let startedAt: Date
}

private struct SmartTubeEDRPressModifier: ViewModifier {
    let enabled: Bool
    let cornerRadius: CGFloat

    @Environment(\.colorScheme) private var colorScheme
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @State private var events: [SmartTubePressPulseEvent] = []
                    ...
                    TimelineView(.animation(minimumInterval: 1.0 / 120.0, paused: events.isEmpty)) { timeline in
                        ...
                        if permitsHardwareEDR {
                            ...
                            RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                                .fill(SmartTubeVisualTokens.mint.opacity(0.055 * heat))
                                .overlay {
                                    RoundedRectangle(cornerRadius: cornerRadius, style: .continuous)
                                        .stroke(SmartTubeVisualTokens.mint.opacity(0.30 * heat), lineWidth: 1.5)
                                }
                        }
                    }
    ...
    private func triggerPulse() {
        let event = SmartTubePressPulseEvent(startedAt: Date())
        events.append(event)
        Task { @MainActor in
...
private struct SmartTubeEDRPulseLayer: UIViewRepresentable {
    let heat: Double
    let cornerRadius: CGFloat
    let permitsEDR: Bool

    func makeUIView(context: Context) -> SmartTubeEDRPulseView {
        SmartTubeEDRPulseView()
    }

    func updateUIView(_ view: SmartTubeEDRPulseView, context: Context) {
...
private final class SmartTubeEDRPulseView: UIView {
...
extension View {
    @ViewBuilder
    func smartTubeEDRPressEffect(enabled: Bool, cornerRadius: CGFloat = 14) -> some View {
        #if os(iOS)
        modifier(SmartTubeEDRPressModifier(enabled: enabled, cornerRadius: cornerRadius))
        #else
        self
        #endif
    }
