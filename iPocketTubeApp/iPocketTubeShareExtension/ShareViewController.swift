...
final class ShareViewController: UIViewController {
    ...
    private func extractAndPrepare() async {
        ...
        guard let link = URL(string: "ipockettube://video/\(videoID)") else {
        ...
        for (i, item) in items.enumerated() {
            ...
            for (j, provider) in attachments.enumerated() {
                ...
                if let id = await resolver.resolve(url: url, onProgress: progress) {
                    guard let link = URL(string: "ipockettube://video/\(id)") else { continue }
                    return (id, link)
                }
            }
        }
    ...
    private func cancel() {
        ...
        extensionContext?.cancelRequest(
            withError: NSError(domain: "com.void.ipockettube.share", code: 0, userInfo: nil)
        )
    }
}
