...
private typealias VideoFormat = iPocketTubeCore.VideoFormat
...
extension PlaybackViewModel {
    ...
    private func parseHLSAudioLanguages(from manifest: String) -> [AudioTrack] {
        iPocketTubeCore.parseHLSAudioLanguages(from: manifest)
    }
    ...
    private func parseHLSVariantURLsForLanguage(
        ...
        iPocketTubeCore.parseHLSVariantURLsForLanguage(contentID, from: manifest, baseURL: baseURL)
    }
    ...
    private func extractQuotedHLSAttribute(_ name: String, from line: String) -> String? {
        iPocketTubeCore.extractQuotedHLSAttribute(name, from: line)
    }
    ...
    private func parseHLSAllVariants(from manifest: String, baseURL: URL) -> [Int: URL] {
        iPocketTubeCore.parseHLSAllVariants(from: manifest, baseURL: baseURL)
    }
    ...
    private func parseHLSBestVariant(from manifest: String, baseURL: URL, minHeight: Int) -> URL? {
        let result = iPocketTubeCore.parseHLSBestVariant(from: manifest, baseURL: baseURL, minHeight: minHeight)
        if let result {
...
/// via a custom `ipockettubehls://` URL scheme. Simulator-only.
///
...
private final class HLSVariantProxy: NSObject, AVAssetResourceLoaderDelegate, @unchecked Sendable {
    ...
    /// Returns a unique `ipockettubehls://` URL. Each call produces a new value to prevent caching.
    static func makeProxyURL() -> URL {
        let ts = UInt64(Date().timeIntervalSince1970 * 1_000)
        return URL(string: "ipockettubehls://variant/\(ts).m3u8")!
    }
