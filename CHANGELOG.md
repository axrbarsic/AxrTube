...
### Изменено
- В настройках появился включённый по умолчанию режим «Только русскоязычные ролики» для явного поиска. Запрос можно вводить на любом языке; кандидаты проверяются по доступным YouTube-метаданным аудиодорожек и автоматических субтитров, недоказанные результаты исключаются, а отключение режима возвращает полный поиск. Для коротких страниц фильтр ограниченно добирает следующие страницы без дублей и пустых промежутков.
- В «Медиатеке» технический раздел RSS заменён на «Каналы»: авторизованный пользователь видит стабильный алфавитный список своих подписок YouTube и может открыть ленту выбранного канала. RSS-модели и локальные данные сохранены для совместимости, но больше не показываются в основной навигации.
- Пользовательские действия получили единую семантически градуированную тактильную отдачу: лёгкий выбор для навигации и настроек, отдельный транспортный импульс, предупреждение для повтора, подтверждение успеха/ошибки и выраженный отклик для удаления. Рендер, прокрутка, пассивный прогресс и непрерывное движение ползунка исключены.
- В настройках появился живой выбор оформления «Системная / Тёмная / Светлая». Matrix-интерфейс теперь использует отдельные семантические палитры для светлого и тёмного режимов, а Share Extension следует тому же сохранённому выбору.

### Исправлено
- Выход из аккаунта стал линейной транзакцией: refresh, обмен YouTube cookies, запись Keychain и распространение авторизации в каталоги/плеер используют один версионированный снимок. Запоздавшие callbacks старой сессии игнорируются, удаление защищённых данных проверяется до окончательного статуса выхода, а обычные вход, обновление токена и восстановление после запуска сохранены.
- Устранены связанные гонки прогрессивного аудио и системной аудиосессии: первый тап после перезапуска теперь атомарно забирает незавершённую загрузку у supervisor, подтверждённые диапазоны переживают отменённые и опоздавшие HTTP-ответы, прямой M4A финализируется без второй постоянной копии, а пересекающиеся interruption и spoken-audio hint больше не дают преждевременный либо двойной resume. Legacy video/HLS recovery больше не подменяет активный progressive-audio item.
- «Главная» больше не исчезает и не пересобирает уже видимые карточки при обновлении или подгрузке: новые страницы добавляются атомарно и без дублей, устаревшие ответы игнорируются, повторный вызов видимого нижнего маркера не запускает рекурсивную пагинацию, а Shorts не расходуют continuation основной ленты.
- Поиск больше не оставляет старую выдачу под видом главной ленты после очистки запроса: результаты, пагинация и фильтры сбрасываются атомарно, discovery обновляется отдельно, а pull-to-refresh повторяет только текущий контекст. Пустые и дублирующиеся идентификаторы карточек удаляются до SwiftUI-layout, поэтому после фильтрации не остаются гигантские вертикальные провалы.
- Лента канала переиспользует общие правила дат публикации, сортировки новых роликов выше старых, скрытия Shorts и компактных карточек медиатеки; добавлены понятные русские состояния загрузки, пустого результата, ошибки и выхода из аккаунта.
- Исправлена общая регрессия дат публикации: неудачная подгрузка точной даты больше не стирает достоверную относительную подпись YouTube, а объединение дублей выбирает данные в порядке «точная дата → относительная дата → неизвестно». Поддержаны текущие `publishedTimeText`, `dateText`, `lockupViewModel` и точная дата из player microformat.
- Ленты Главной, рекомендаций, подписок, каналов и поиска теперь действительно пересортировываются от новых роликов к старым после пакетного уточнения metadata; честные относительные подписи участвуют в порядке как типизированный примерный возраст, а неизвестные даты остаются стабильным блоком внизу. История сохраняет порядок просмотра, а пользовательские плейлисты — заданный порядок элементов.
- Для новых вариантов channel/playlist/lockup renderer добавлен общий поиск публикационной подписи и резервная точная дата из публичной watch-page metadata. Строки каталога плейлистов больше не выдают дату самого плейлиста за дату публикации ролика и не показывают ложное «Дата неизвестна».
