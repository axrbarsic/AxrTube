...
15. **Экспериментальный Dynamic Island:** добавлен сохраняемый выбор `Выключено / Минимальный / Прогресс / Волна / Строка / Авто`. Playback start, pause, resume, stop и смена ролика обновляют одну playback Live Activity, а перезапуск удаляет stale экземпляры. Волна является лёгким snapshot-паттерном с ограниченной cadence, captions получают безопасный fallback на metadata.
...
17. **Публичный GitHub AxrTube:** имя репозитория, README, privacy и support links, шаблоны, topics и галерея актуального интерфейса iOS 26 синхронизированы с текущим продуктом. Исторические технические identifiers оставлены только там, где они нужны для совместимости подписи, extensions, deep links, кэша и данных приложения.
