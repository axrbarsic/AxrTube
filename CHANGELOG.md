...
15. **Экспериментальный Dynamic Island:** варианты playback Live Activity были проверены, после физической оценки отдельная playback-карточка выведена из активного flow как дублирующая системный Now Playing. Lifecycle cleanup завершает её старые экземпляры.
