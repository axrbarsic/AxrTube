...
4. **Оформление:** добавлен живой выбор «Системная / Тёмная / Светлая», отдельные семантические палитры Matrix и согласованное оформление Share Extension.
...
8. **Liquid Glass:** на iOS 26 системное стекло применяется к tab bar, поиску, mini-player и основным действиям. Светлая тема получила нейтральный content layer, Matrix сохранила тёмный зелёный фон, а обычные карточки не превращаются в конкурирующие стеклянные панели.
...
15. **Экспериментальный Dynamic Island:** после физической оценки пользовательский flow сокращён до одного progress-стиля. По умолчанию остаётся только системный Now Playing. Отдельный переключатель включает круговой minimal-индикатор и длинную compact-полосу, где зелёный показывает просмотренную часть, а жёлтый оставшуюся. Настройка честно предупреждает об обязательной карточке ActivityKit на Lock Screen. Lifecycle cleanup завершает старые и дублирующиеся экземпляры.
...
19. **Управляемая карточка на Lock Screen:** по умолчанию отдельная playback Live Activity выключена, чтобы не дублировать системный Now Playing. Пользователь может явно включить единственный progress-стиль Dynamic Island, понимая системную границу ActivityKit. Download-only Live Activity не затронута. Готовая стенограмма может дать одну короткую русскую выжимку в стандартном поле metadata. В настройках вручную выбираются Gemini, DeepSeek или Groq, конкретная модель и отдельный ключ в Keychain. Автоматического fallback между провайдерами нет. Кэш учитывает провайдер и модель, а `retry-after` показывает спокойный cooldown вместо красной ошибки и повторных запросов.
