...
- **Ранний старт на слабой сети:** исправлена политика AVPlayer, которая на канале медленнее bitrate могла ждать почти полную загрузку ради минимизации stall. Первый звук теперь разрешён на первых playable bytes, а после фактического движения таймлайна возвращается системное осторожное ожидание. Новый ручной «Режим слабой сети» сохраняется между запусками и выбирает минимальный совместимый AAC/M4A bitrate для playback, автосохранения и ручной загрузки аудио. Заголовок, индекс и ближайший диапазон по-прежнему имеют приоритет, а оставшийся файл заполняется небольшими возобновляемыми блоками.
