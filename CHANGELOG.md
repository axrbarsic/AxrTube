...
### Полные заголовки компактных карточек, 9 сентября 2026

- В компактной ленте и загрузках убран лимит строк заголовка. Маленькое превью сохранено; текст переносится, увеличивая высоту карточки без многоточия.
- Компактные хронологические и декоративные карточки также не ограничивают число строк. Компактный постер использует текст рядом с превью вместо обрезаемого блока поверх изображения.
- Подписанная сборка прошла, установлена по USB поверх приложения и запущена. Пользовательская визуальная приёмка ожидается; данные сохранены.

### Переключение офлайн-роликов и сетка, 8 сентября 2026, ожидает визуальной проверки

- Убраны внешние ленивые контейнеры вокруг уже ленивых сеток канала и каталога. Высота обычной карточки ограничена её содержимым. Устранение пустых промежутков ещё требует визуальной проверки на исходном маршруте.
- Смена ролика сбрасывает ожидание качества предыдущего item. Готовый локальный файл открывается до сетевой подготовки; отменённый или устаревший load не может подменить текущий ролик.
- Старое событие currentItem больше не отменяет подключение PCM-визуализации нового item. Завершение перемотки проверяет item и считывает реальную позицию плеера; поздние события локального окончания и восстановления тоже проверяют владельца.
- 14 адресных тестов прошли. После финальных защит повторены 4 теста таймлайна, все прошли, включая реальный AVPlayer: A, B, сброс ожидания качества и перемотка на 20-ю секунду. Один simulator build прошёл, приложение установлено поверх данных и запущено.
- 9 сентября подписанная iOS-сборка проверена, установлена поверх приложения по USB и запущена на физическом iPhone без удаления данных. Wi-Fi-соединение перед этим было недоступно. Код опубликован как кандидат обновления; визуальная приёмка ожидает вердикта пользователя.
...
- На этапе 8 сентября эта правка проверена без подключения к физическому iPhone; 9 сентября она вошла в установленное обновление. Слышимость звука и реальные аудиопрерывания не объявляются подтверждёнными по таймлайну или сигналу.
