...
- **Progressive audio и офлайн.** AVPlayer получает прямой HTTPS media URL и сам планирует нужные для раннего звука HTTP Range-запросы. Офлайн-сохранение больше не стоит между плеером и сетью: отдельный durable sparse downloader запускается после доказанного первого звука либо по ограниченному fallback-таймеру. Поэтому воспроизведение не ждёт полной офлайн-копии. Если временная YouTube-ссылка перестала работать, direct item ограниченно обновляется; если локальный файл уже готов, AxrTube переключается на него с сохранением позиции и намерения Play/Pause. Переключатель «Минимум / Обычное» находится прямо в баннере текущего трека и на активной карточке загрузки. Минимальный режим выбирает самый маленький доступный AAC/M4A bitrate. Карточка показывает живое короткое окно реального аудиосигнала и отдельный ползунок перемотки. Позиция каждого ролика сохраняется отдельно и восстанавливается перед следующим запуском.
- **Устойчивая многосегментная докачка.** Пока AVPlayer не доказал стабильный buffer, офлайн-загрузчик использует только одно соединение. После нескольких последовательных healthy samples обычный режим может запросить до восьми непересекающихся HTTP Range-сегментов одновременно (минимальный режим сохраняет один-два фоновых потока). Чанки докачки — по 2 МБ: YouTube обслуживает ограниченные range-запросы на полной скорости, тогда как один большой GET троттлится до скорости воспроизведения. Сервер без Range автоматически остаётся на последовательной загрузке. Все ответы записываются одним владельцем в тот же `.part`, metadata refresh выполняется single-flight, а подтверждённые блоки переживают потерю сети и перезапуск. Стабильная identity ролика и representation не смешивает разные bitrate при обновлении временной подписи YouTube URL. Wi-Fi-only остаётся сетевым ограничением именно для офлайн-докачки.
...
AxrTube основан на проекте [milika/SmartTubeIOS](https://github.com/milika/SmartTubeIOS); исходные copyright notices и история upstream сохранены. Ранний playback использует системный HTTPS scheduler AVPlayer. Возобновляемый sparse downloader работает отдельно и с меньшим приоритетом, атомарно финализирует офлайн-файл и не блокирует первый звук.
