...
Open-source видеоплеер и офлайн-медиатека вокруг YouTube для iPhone. Обычный тап открывает нативное видео, а ролик можно сохранить целиком и воспроизводить без интернета.
...
- **Офлайн-медиатека по архитектуре Brave Playlist.** AxrTube предпочитает HLS и сохраняет его системной `AVAssetDownloadURLSession`; прямой muxed MP4 загружается фоновой `URLSessionDownloadTask`. Task identity восстанавливает незавершённую системную задачу после relaunch, прогресс и локальная ссылка сохраняются в одном manifest, а повторный тап не создаёт дубль.
...
AxrTube основан на проекте [milika/SmartTubeIOS](https://github.com/milika/SmartTubeIOS); исходные copyright notices и история upstream сохранены. Архитектура фонового HLS/file download и восстановления задач адаптирована из открытого Brave iOS Playlist с сохранением заголовков Mozilla Public License 2.0 в производных файлах.
