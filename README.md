...
- **Устойчивая многосегментная докачка.** Пока AVPlayer не доказал стабильный buffer, офлайн-загрузчик использует только одно соединение. После нескольких последовательных healthy samples обычный режим может запросить до четырёх непересекающихся HTTP Range-сегментов одновременно. Минимальный режим сохраняет один фоновый поток, а сервер без Range автоматически остаётся на последовательной загрузке. Все ответы записываются одним владельцем в тот же `.part`, metadata refresh выполняется single-flight, а подтверждённые блоки переживают потерю сети и перезапуск. Стабильная identity ролика и representation не смешивает разные bitrate при обновлении временной подписи YouTube URL. Wi-Fi-only остаётся сетевым ограничением именно для офлайн-докачки.
